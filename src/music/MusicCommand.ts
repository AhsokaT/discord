...
export abstract class MusicCommand extends Command {
    constructor(ctx: Command.LoaderContext, options: Command.Options) {
        super(ctx, {
            ...options,
            preconditions: options.preconditions ? ['CachedGuild', ...options.preconditions] : ['CachedGuild']
        });
    }

    abstract run(interaction: Command.ChatInputCommandInteraction<'cached'>, subscription: Subscription): void;
    ...
    async chatInputRun(interaction: Command.ChatInputCommandInteraction<'cached'>) {
        ...
        if (!subscription) {
            ...
            for (const [, command] of guild.commands) {
                ...
                return void interaction.reply({ content: `Nothing is playing! You can use </play:${command.id}> to play music.`, ephemeral: true }).catch(console.error);
            }

            return void interaction.reply({ content: 'Nothing is playing!', ephemeral: true }).catch(console.error);
        }
