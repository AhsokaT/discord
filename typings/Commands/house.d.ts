import { ApplicationCommandDataResolvable, ButtonInteraction, Interaction, SelectMenuInteraction } from 'discord.js';
import { Command } from '../client';
export declare enum House {
    TIGER = "\uD83D\uDC2F House of Tiger",
    OWL = "\uD83E\uDD89 100 Acre Wood",
    RAVEN = "\uD83D\uDC41\uFE0F The Ravens",
    TURTLE = "\uD83D\uDC22 Kame House",
    PANDA = "\uD83D\uDC3C Bamboo Forest"
}
export declare enum RoleID {
    TIGER = "1024014286416261191",
    OWL = "1024014430448660490",
    RAVEN = "1024014477789773965",
    TURTLE = "1024014510723432478",
    PANDA = "1024014614536667239"
}
export declare class HouseCommand implements Command {
    get names(): string[];
    get guilds(): string[];
    get commandBuilders(): ApplicationCommandDataResolvable[];
    receive(interaction: Interaction<'cached'>): void;
    receiveButton(interaction: ButtonInteraction<'cached'>): Promise<void>;
    receiveSelectMenu(interaction: SelectMenuInteraction<'cached'>): undefined;
}
