...
exports.HOUSE_INFO = new template_1.Command()
    ...
    const value = houses.length > 1 ? `Joint **${Ordinal[position + 1]}** with ${houses.filter(([name]) => name !== house).map(([name]) => `<@&${housePicker_1.RoleID[name]}>`).join(', ')}` : `**${Ordinal[position + 1]}** with **${pointManager.cache[house]} points**`;
