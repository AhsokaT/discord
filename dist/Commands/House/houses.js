...
exports.HOUSES = new template_1.Command()
    ...
    if (false && interaction.member.roles.cache.hasAny(housePicker_1.RoleID.TIGER, housePicker_1.RoleID.OWL, housePicker_1.RoleID.RAVEN, housePicker_1.RoleID.TURTLE, housePicker_1.RoleID.PANDA))
    ...
    if (interaction.customId.startsWith('CHOOSEHOUSE')) {
        ...
        actionRow.addComponents(buttons);
        interaction.update({
            content: 'You can choose your house at any time by using the command `/choosehouse`',
            components: [actionRow]
        });
    }
});
